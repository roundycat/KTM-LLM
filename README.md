...
> 📚 **전체 실험·결과·인수인계는 → [HANDOVER.md](HANDOVER.md)** (모델 6종·방법 6종·전체 517 결과·해설
> 생성/검증·트러블슈팅·재현·체크리스트·프롬프트/하이퍼파라미터 부록). 개발 로그 → [EXPERIMENTS.md](EXPERIMENTS.md),
> 해설 의학정확성 검증 → [해설_검증_리포트.md](dataset/해설_검증_리포트.md).
> 아래 표는 **메인(용어 RAG) 결과만** 요약이며, 추론기법(CoT/SC, Qwen +4.06%p)·환각완화(vote3)·추가
> 국내모델(SOLAR)·가설검증(EXAONE vs Llama)은 HANDOVER/EXPERIMENTS에 있습니다.
...
> 생성한 뒤, **다중 에이전트 적대 검증**(517 전수 → 2차 독립 재검증)으로 **확정 사실오류 8건(중대 2)을
> 교정**했습니다(정확도 ≈98.5%). 상세 → [해설_검증_리포트.md](dataset/해설_검증_리포트.md),
> 하니스 → `scripts/verify_explanations_*`.
