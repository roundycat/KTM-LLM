...
> ⚠️ **아카이브(원본 스냅샷)입니다. 정본은 [`graphrag/`](../graphrag/) 입니다.**
> 이 `bigse0u1/` 디렉터리는 통합 전 원본 협업 브랜치의 스냅샷으로 보존용입니다.
> 이후 리팩터·보강(보기Graph 방식, 청크 임상 재작성, 평가 지표 등)은 모두 `graphrag/`에
> 반영되어 있으니, **실행·재현·인용은 `graphrag/`를 사용하세요.** 두 디렉터리의
> `step3_graphrag_query.py`·`data/kg_all_*.jsonl`은 분기된 복사본이라 내용이 다를 수 있습니다.
...
**GraphRAG seed 추출 방식:**
```
질문 임베딩 → 증상 노드 3,873개와 BGE-m3 유사도 비교
→ 상위 10개 증상 노드 선택 (LLM 번역 불필요)
→ Neo4j: 해당 증상에 연결된 처방 탐색
→ 벡터 검색 결과와 합쳐 LLM에 전달
```
...
전체 문제에서는 방식 간 차이가 거의 없음(59~60%). LLM이 이미 강한 지식을 갖고 있어 RAG의 추가 효과가 희석되기 때문. 처방형 문제에서만 방식 간 차이가 두드러짐.

---

### 처방형 문제 86개 기준 (`--rx-only --k 20`)

#### 단계별 개선 요약 (gpt-4o-mini)

| 단계 | 주요 변경 | 그냥 LLM | 벡터 RAG | GraphRAG | recall@ctx |
|------|---------|---------|---------|---------|-----------|
| 초기 (k=5) | — | 30.2% | 29.1% | 31.4% | 4.7% |
| 임상 청크 재작성 + k=20 | symptom-first 청크, k 확대 | 30.2% | 37.2% | 30.2% | 7.0% |
| **누락 처방 추가 + seed 벡터화** | 24개 처방 DB 보완, LLM→벡터 추출 | 30.2% | **38.4%** | **43.0%** | **12.8%** |

---

#### 최종 결과 (gpt-4o-mini, k=20, seed 벡터화)
...
### GraphRAG seed 벡터화로 성능이 크게 오른 이유

초기 GraphRAG는 LLM이 시험 문제의 임상 표현을 한의학 한문 용어로 번역한 뒤 그래프 노드와 텍스트 매칭하는 방식이었습니다. 번역이 틀리면 그래프 조회 결과가 전혀 없거나 엉뚱한 처방을 반환해 성능이 낮았습니다.

seed 벡터화 이후에는 질문 임베딩과 증상 노드 임베딩을 직접 비교해 번역 단계를 없앴습니다.

| 개선 요인 | 효과 |
|---------|------|
| seed 벡터화 (LLM 번역 제거) | GraphRAG 30.2% → **43.0%** (+12.8%p) |
| 임상 청크 재작성 (증상 우선 배치) | 벡터 RAG recall 향상 |
| 누락 처방 24개 추가 | recall@ctx 4.7% → **12.8%** |
